...
gg <- plotMaterial(leaf.tibl, label="leaf mineral composition ", path=output.path, xaxis, material=leafMaterial, material.history=material.tibl)
...
plotMaterial <- function(tibble, label, path, xaxis, material, material.history) {
  ...
  leafPhelps.tibl <- tibble %>% dplyr::filter(ranch == 'phelps')
  leafHuff.tibl <- tibble %>% dplyr::filter(ranch == 'huff')
  ...
  gg <- ggplot(leafPhelps.tibl, aes(x = as.character(year), y = Potassium ) ) +
    ...
    scale_y_continuous(limits=c(.5, 3.75), 
                       breaks = seq(min(0), max(as.numeric(material.high) + .15), 
                                by = .25)) +
