...
source.url <- c("https://raw.githubusercontent.com/cordphelps/soil-leaf-reports/master/dataCSV/leafData.csv")

leaf.tibl <- dplyr::as_tibble(read.csv(file=source.url, header=TRUE, row.names=NULL))

leafMaterial <- c("Potassium")

gg <- plotMaterial(leaf.tibl, label="test", path=output.path, xaxis, material=leafMaterial)
...
plotMaterial <- function(tibble, label, path, xaxis, material) {
  ...
  # gg <- ggplot(tibble, aes(x = year, y = !!sym(material) ) ) + 
    ...
    gg <- ggplot(tibble, aes(x = as.numeric(year), y = Potassium ) ) +
    ...
    scale_x_discrete(limits=c(2012, 2013, 2018, 2019, 2020, 2021, 2022)) +
      
    #scale_y_continuous(breaks = seq(min(0), max(5), by = 1)) +
    ...
    content.string <- paste("K", "400", " gal", sep="")
    gg <- gg + makeLabels(content=content.string, x.position=2013, y.position = 4)
